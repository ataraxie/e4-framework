...
if !results_dir || !statement_key || !["nodespread", "vanilla_times_taken", "livelyblogs_times_taken"].include?(statement_key)
...
statements['livelyblogs_times_taken'] = "SELECT CAST(ROUND(AVG(time_taken), 0) as int) as average_time_taken FROM E4Measurement WHERE virtual_user IN ('LivelyBlogNavigator','LivelyBlogSearcher','LivelyBlogMacroPageCreator','LivelyBlogMacroPageReader','LivelyBlogPostCreator','LivelyBlogPageToBlogpostConvertor') GROUP BY virtual_user ORDER BY virtual_user;"
...
envs.each do |env|
  ...
  result = `sqlite3 -list #{sqlite_file} "#{statement}"`
  result = result.split("\n")
  puts result
  if statement_key.end_with?("times_taken")
